...
- **Profiling before optimising** (`python scripts/profile_mining.py`): staged wall-clock attribution *plus* a cProfile hotspot ranking. It exists because the honest answer to "should this be C++?" is usually "no": with it, work moved to the places where measurement said the time actually goes, and stayed in numpy where the bottleneck was already a C kernel.
