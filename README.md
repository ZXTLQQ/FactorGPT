...
- **What it does not provide**: industry/market-cap/financial/news fields — these are not derivable from the price-volume source, so neut/alternative-data dimensions degrade gracefully to empty and the factor pipeline still runs on pure price-volume data plus the benchmark index series.
